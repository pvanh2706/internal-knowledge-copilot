...
$SmokeRoot = Join-Path $env:TEMP ("ikc-m5-smoke-" + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    $Folder = Invoke-RestMethod `
        ...
        -Headers $Headers `
        -ContentType 'application/json' `
        -Body '{"parentId":null,"name":"M5 Smoke"}'
    ...
    if ($Answer.needsClarification -or @($Answer.citations).Count -lt 1 -or -not $Answer.answer.Contains('Payment error')) {
        Write-Host '--- API LOG TAIL ---'
        Get-Content $ApiLog -Tail 180
        Write-Host '--- API ERR TAIL ---'
        Get-Content $ApiErr -Tail 180 -ErrorAction SilentlyContinue
