# Hướng Dẫn Tích Hợp Và Triển Khai Kỹ Thuật Cho Bên Thứ 3
...
- Cấu hình tenant, application, integration connection, network, CORS, JWT, storage, vector DB và AI provider để bên thứ 3 có thể kết nối an toàn.
- Chuẩn bị checklist kỹ thuật trước khi go-live.

Đối tượng đọc chính:

- Team kỹ thuật triển khai Internal Knowledge Copilot.
- Team kỹ thuật của hệ thống đối tác cần đẩy dữ liệu vào Copilot hoặc cho Copilot gọi ngược.
- Solution architect/technical consultant cần tư vấn phạm vi chức năng, ranh giới tích hợp và điều kiện vận hành.

## A. Tóm Tắt Chức Năng Có Thể Tư Vấn

| Nhóm chức năng | Mục tiêu tư vấn | Cách tích hợp hiện tại |
| --- | --- | --- |
| Quản trị tri thức nội bộ | Upload tài liệu, duyệt, xử lý thành tri thức, hỏi AI có citation. | Dùng JWT user/admin/reviewer, upload qua `POST /api/documents`, approve rồi worker index. |
| AI Q&A theo quyền | User hỏi trên tài liệu/wiki/object được phép xem. | Gọi `POST /api/ai/ask` bằng JWT; backend lọc theo tenant, folder/document và ACL external. |
| Đồng bộ object nghiệp vụ | Đưa metadata object từ CRM/ticketing/wiki portal vào Copilot. | Gọi `POST /api/integrations/{applicationCode}/objects/sync` bằng integration key. |
| Đồng bộ quyền external | Cho Copilot lọc retrieval/recommendation theo quyền của hệ thống gốc. | Gọi `POST /api/integrations/{applicationCode}/permissions/sync`; worker cập nhật ACL snapshot. |
| Event nghiệp vụ | Lưu event từ hệ thống gốc để audit/trigger xử lý về sau. | Gọi `POST /api/integrations/{applicationCode}/events`; hiện lưu inbound event. |
| Workflow copilot | Gợi ý bước tiếp theo cho case CRM deal stage changed, kèm nguồn tham chiếu. | Gọi `POST /api/workflow-copilot/deal-stage-changed` bằng JWT user. |
| Audit và vận hành | Theo dõi upload, approve, event, AI interaction, job lỗi. | Dùng audit log, operation metrics, processing jobs và smoke test sau triển khai. |

Thông điệp tư vấn quan trọng:

- Copilot không thay thế hệ thống nguồn. Hệ thống nguồn vẫn là source of truth cho dữ liệu nghiệp vụ, quyền và action cuối cùng.
- Copilot phù hợp nhất khi dữ liệu quy trình/tri thức đã có owner rõ ràng, có reviewer duyệt, và có mapping user/group ổn định.
- Vector DB không phải lớp bảo mật. Quyền phải được kiểm tra lại bằng SQLite ACL, folder permission hoặc realtime connector.

## B. Phương Án Tích Hợp Đề Xuất

### Phương án 1. Tích hợp nhanh bằng upload tài liệu

Phù hợp khi bên thứ 3 chỉ cần đưa tài liệu/quy trình vào Copilot để hỏi đáp AI.

```text
Hệ thống đối tác/operator
-> login lấy JWT
-> upload file vào folder
-> reviewer approve
-> worker extract/chunk/embed/index
-> user hỏi AI
```

Ưu điểm:

- Ít phụ thuộc kỹ thuật giữa hai hệ thống.
- Dễ pilot với file `.pdf`, `.docx`, `.md`, `.txt`.
- Phù hợp cho tài liệu quy trình, chính sách, hướng dẫn nội bộ.

Giới hạn:

- Chưa phải sync realtime từ hệ thống gốc.
- Cần reviewer/admin approve trước khi thành tri thức chính thức.

### Phương án 2. Tích hợp event-driven metadata và ACL

Phù hợp khi bên thứ 3 muốn Copilot biết object nào tồn tại và user nào được xem.

```text
Hệ thống đối tác
-> objects/sync
-> permissions/sync
-> Copilot worker cập nhật external_objects + external_acl_snapshots
-> AI/workflow lọc quyền theo snapshot và revalidate khi có connector
```

Ưu điểm:

- Có ranh giới tenant/application rõ ràng.
- Có idempotency key để retry an toàn.
- Dùng được cho CRM deal, ticket, procedure, contract hoặc object nghiệp vụ khác.

Giới hạn:

- `documents/changed` hiện mới lưu event, chưa tự kéo content và index external document.
- Nếu cần index content realtime, nên bổ sung worker gọi API content của đối tác hoặc dùng upload API hiện có.

### Phương án 3. Tích hợp sâu bằng outbound connector

Phù hợp khi Copilot cần gọi ngược sang hệ thống gốc để lấy context, kiểm tra quyền realtime hoặc thực thi action.
...
- Quyền nhạy cảm được kiểm tra realtime.
- Workflow recommendation có context mới nhất từ hệ thống nguồn.
- Action execution vẫn do hệ thống gốc kiểm soát.

Giới hạn triển khai hiện tại:

- Secret inbound được lưu dạng hash để xác thực bên thứ 3 gọi vào Copilot, vì vậy backend hiện không lấy lại được secret thô để gửi outbound.
- Connector outbound trong code hiện truyền `X-Integration-Key-Id`; `X-Integration-Key` chỉ được gửi nếu implementation cấp được `ApiKey` vào `ExternalConnectorContext`.
- Khi triển khai sâu, cần chọn một trong các hướng: cho đối tác allowlist `X-Integration-Key-Id` + network/TLS, bổ sung outbound secret store riêng, hoặc chuyển sang cơ chế ký request/OAuth phù hợp.

## C. Kiến Trúc Triển Khai Tích Hợp
...
Reverse proxy / IIS
        |
        v
ASP.NET Core API + static frontend
        |
        +--> SQLite metadata DB
        +--> Local/private file storage
        +--> ChromaDB vector store
        +--> AI provider hoặc OpenAI-compatible endpoint
        +--> Partner baseUrl /copilot/... nếu bật outbound connector
```

Các ranh giới kỹ thuật:

| Thành phần | Vai trò | Lưu ý triển khai |
| --- | --- | --- |
| IIS/reverse proxy | Terminate TLS, public endpoint cho UI/API. | Chỉ publish API/frontend, không publish storage hoặc Chroma. |
| ASP.NET Core API | Xử lý auth, tenancy, document, AI, integration, worker. | App pool cần quyền đọc/ghi DB, storage, log. |
| SQLite | Source of truth metadata nghiệp vụ. | Backup cùng storage; cân nhắc SQL Server/PostgreSQL khi tải enterprise lớn. |
| Storage | Lưu file upload, extracted text, normalized text. | Đặt ngoài web root, không serve trực tiếp. |
| ChromaDB | Lưu vector chunks phục vụ semantic retrieval. | Chỉ API server truy cập, không mở ra internet. |
| AI provider | Embedding, answer generation, understanding, workflow recommendation. | Dùng mock cho test; production cần key/provider thật và kiểm soát dữ liệu gửi ra. |
| Partner API | Cung cấp context, permission check, action endpoint cho Copilot gọi ngược. | Nên có HTTPS, timeout rõ ràng, idempotency và audit ở hệ thống nguồn. |

## D. Cấu Hình Hệ Thống Để Bên Thứ 3 Kết Nối

### D.1 Biến môi trường bắt buộc/khuyến nghị

Các biến cấu hình nên đặt ở IIS app pool environment, server secret hoặc secret manager; không commit secret thật vào repo.

| Nhóm | Key | Ví dụ | Ghi chú |
| --- | --- | --- | --- |
| ASP.NET | `ASPNETCORE_ENVIRONMENT` | `Production` | Production không bật seed/dev behavior. |
| ASP.NET | `ASPNETCORE_URLS` | `http://localhost:5000` | Khi chạy sau IIS có thể do hosting bundle quản lý. |
| Database | `Database__SqlitePath` | `D:\IKCData\db\internal-knowledge-copilot.db` | Thư mục phải writable. |
| Storage | `Storage__RootPath` | `D:\IKCData\storage` | Đặt ngoài site public. |
| Storage | `Storage__MaxUploadBytes` | `20971520` | Đồng bộ với giới hạn reverse proxy/IIS. |
| JWT | `Jwt__Issuer` | `InternalKnowledgeCopilot` | Phải khớp token validation. |
| JWT | `Jwt__Audience` | `InternalKnowledgeCopilot` | Phải khớp client/token. |
| JWT | `Jwt__SigningKey` | `<server-secret>` | Ít nhất 32 ký tự, rotate theo quy trình. |
| CORS | `Cors__AllowedOrigins__0` | `https://copilot.company.vn` | Thêm domain frontend/portal được phép gọi API từ browser. |
| Chroma | `Chroma__BaseUrl` | `http://localhost:8000` | Chỉ API server truy cập. |
| Chroma | `Chroma__Collection` | `knowledge_chunks_prod` | Đổi collection khi đổi embedding dimension. |
| AI | `AiProvider__Name` | `openai-compatible` | `mock` chỉ dùng test/local. |
| AI | `AiProvider__ApiKey` | `<secret>` | Key LLM, không log. |
| AI | `AiProvider__ChatModel` | `<chat-model>` | Chọn model theo môi trường triển khai. |
| AI embedding | `AiProvider__EmbeddingProviderName` | `openai-compatible` | Có thể khác chat provider. |
| AI embedding | `AiProvider__EmbeddingModel` | `text-embedding-3-large` | Phải khớp `EmbeddingDimension`. |
| AI embedding | `AiProvider__EmbeddingDimension` | `3072` | Không trộn chunk cũ khác dimension trong cùng collection. |
| Worker | `BackgroundJobs__PollSeconds` | `5` | Tần suất worker lấy job. |
| Worker | `BackgroundJobs__MaxAttempts` | `3` | Retry mặc định; một số job có rule riêng. |
| Seed | `Seed__Enabled` | `false` | Production nên tắt seed user mẫu. |
| Data reset | `DataReset__Enabled` | `false` | Chỉ bật tạm trong môi trường test. |

Ví dụ cấu hình production tối thiểu:
...
Chroma__Collection=knowledge_chunks_prod
AiProvider__Name=openai-compatible
AiProvider__ApiKey=<llm-secret>
AiProvider__EmbeddingProviderName=openai-compatible
AiProvider__EmbeddingApiKey=<embedding-secret>
Seed__Enabled=false
DataReset__Enabled=false
```

### D.2 CORS và public URL

Nếu bên thứ 3 gọi API từ backend server-to-server, CORS không ảnh hưởng. Nếu gọi từ browser hoặc portal nhúng, cần thêm origin của portal vào:

```json
{
  "Cors": {
    "AllowedOrigins": [
      "https://copilot.company.vn",
      "https://partner-portal.company.vn"
    ]
  }
}
```

Không dùng wildcard origin cho production nếu API có JWT hoặc dữ liệu nội bộ.

### D.3 Network matrix

| Hướng kết nối | Nguồn | Đích | Port/protocol | Bắt buộc khi nào |
| --- | --- | --- | --- | --- |
| User/UI -> Copilot | Browser/user network | `https://copilot.company.vn` | HTTPS 443 | Luôn cần cho UI/API. |
| Partner -> Copilot | Server đối tác | `/api/integrations/...` | HTTPS 443 | Khi sync event/object/permission. |
| Copilot -> Partner | API server Copilot | `{baseUrl}/copilot/...` | HTTPS 443 | Khi bật realtime context/permission/action. |
| Copilot -> Chroma | API server Copilot | ChromaDB | HTTP nội bộ | Khi dùng RAG/vector retrieval. |
| Copilot -> AI provider | API server Copilot | AI endpoint | HTTPS | Khi dùng model thật. |
| Backup/ops -> data | Ops host | DB/storage backup path | Nội bộ | Khi backup/restore. |

Khuyến nghị bảo mật network:

- Chỉ mở public endpoint qua HTTPS.
- Allowlist IP server đối tác cho integration API nếu có thể.
- Không expose ChromaDB và storage ra internet.
- Ghi log request id/correlation id, nhưng không log JWT, integration secret hoặc raw PII.

### D.4 Cấu hình tenant/application/connection

Trình tự bắt buộc để bên thứ 3 kết nối:

1. Tạo tenant hoặc dùng tenant đã có.
2. Tạo application đại diện cho hệ thống đối tác.
3. Tạo integration connection cho application.
4. Chia sẻ cho bên thứ 3: `baseUrl`, `tenantCode`, `applicationCode`, `secretReference`, secret thật, danh sách endpoint được phép gọi.
5. Test inbound bằng Postman/curl trước khi mở luồng dữ liệu thật.
...
Sau bước này, bên thứ 3 gọi inbound integration API bằng:
...
### D.5 Checklist cấu hình đối tác

Bên thứ 3 cần chuẩn bị:

- Service account hoặc backend job có thể gọi HTTPS tới Copilot.
- Cơ chế giữ secret an toàn, không hard-code trong frontend.
- `tenantCode`, `applicationCode`, `secretReference`, secret thật.
- Quy tắc tạo `idempotencyKey` ổn định cho retry.
- Mapping user/group/role sang `subjectType` và `subjectId`.
- Nếu Copilot gọi ngược: public/internal HTTPS endpoint tại `baseUrl` và các API `/copilot/...`.
- Audit log phía đối tác cho event đã gửi, quyền đã sync, action đã thực thi.
...
- Lọc quyền cho external candidates bằng ACL snapshot; connector revalidation đã có boundary/code gọi ngược nhưng production cần thống nhất auth outbound như mô tả ở mục 8.
...
- Hệ thống gốc vẫn là source of truth. Với dữ liệu nhạy cảm, nên bật revalidation qua connector sau khi đã thống nhất auth outbound.
...
Ghi chú auth outbound hiện tại:

- Copilot luôn có thể gửi `X-Integration-Key-Id` bằng `secretReference` của integration connection.
- `X-Integration-Key` chỉ được gửi nếu implementation có cơ chế cấp lại secret thô vào `ExternalConnectorContext`. Bản hiện tại lưu secret inbound dạng hash nên không tự đọc lại được secret để gọi outbound.
- Nếu triển khai realtime permission/context/action trong production, hai bên cần thống nhất một cơ chế auth cụ thể trước go-live: allowlist theo network + key id, outbound secret store riêng, mutual TLS, OAuth/client credential, hoặc request signing.
...
X-Integration-Key-Id: crm-prod-key-2026-06
X-Integration-Key: <integration_secret_if_outbound_secret_store_enabled>
```
...
Collection chính thức trong repo:

```text
docs/postman/InternalKnowledgeCopilot_ThirdPartyIntegration.postman_collection.json
```

Import file này vào Postman để có sẵn request, biến môi trường, sample payload và ghi chú test cho từng bước tích hợp.

### Environment variables

Collection đã có sẵn collection variables. Có thể chỉnh trực tiếp trong tab Variables của collection hoặc tạo Postman environment riêng:
...
- Với dữ liệu nhạy cảm, hệ thống gốc nên cung cấp `/copilot/permissions/check` và hai bên phải test auth outbound trước go-live.
...
- [ ] Nếu cần, `/copilot/permissions/check` của hệ thống gốc đã sẵn sàng và auth outbound đã được test.
...
- Outbound secret store hoặc cơ chế auth machine-to-machine chuẩn hơn nếu cần realtime connector trong production.
